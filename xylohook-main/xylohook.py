...
black = "\033[1;30m"
titletext = " [-- XYLOHOOK --] Made by github.com/Solvixs"
red = "\033[1;31m"
green = "\033[1;32m"
yellow = "\033[1;33m"
blue = "\033[1;34m"
...
cyan = "\033[1;36m"
white = "\033[1;37m"
invalidurl = f"{red}[! XYLOHOOK !]{white} Invalid url!"
# test = "" test webhook, dont forget to remove :3

socials = {
    "github": {"link": "https://github.com/Solvixs"},
...
$$/   $$/     $$/     $$$$$$$$/  $$$$$$/  
    ...
    >> [Webhook Multitool developed by @xyloq._]
"""
...
# def sendembed(url):
#     tit = input(f"{yellow}[? XYLOHOOK ?]{white}Title for the embed: ")
#     des = input(f"{yellow}[? XYLOHOOK ?]{white}Description: ")
#     color = input(f"{yellow}[? XYLOHOOK ?]{white}Hex-Color: ")
...
def changepfp(url):
    input(f"{yellow}[? XYLOHOOK ?]{white} Press enter to select file or skip this to input the path/url")
    image_path = fd.askopenfilename(filetypes=[("Profile Pictures", "*.png;*.jpg;*.jpeg")])
    if image_path is None or image_path == "":
        clear()
        image_path = input(f"{yellow}[? XYLOHOOK ?]{white} Path/URL to image: ")
    ...
    try:
        ...
        response.raise_for_status()
        print(f"{green}[+ XYLOHOOK +]{white} Profile picture changed successfully.")
    except FileNotFoundError:
        print(f"{red}[! XYLOHOOK !] File not found. Please provide a valid file path or image url.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! XYLOHOOK !] HTTP Error: {errh}")
    except requests.exceptions.ConnectionError as errc:
        print(f"{red}[! XYLOHOOK !] Error Connecting: {errc}")
    except requests.exceptions.Timeout as errt:
        print(f"{red}[! XYLOHOOK !] Timeout Error: {errt}")
    except requests.exceptions.RequestException as err:
        print(f"{red}[! XYLOHOOK !] Request Exception: {err}")

def deletehook(url):
    print(f"{cyan}[+ XYLOHOOK +]{white} Trying to delete webhook...")
    try:
        response = requests.delete(url)
        response.raise_for_status()  # Raises HTTPError for bad responses
        print(f"{green}[+ XYLOHOOK +]{white} Webhook deleted successfully.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! XYLOHOOK !] HTTP Error: {errh}")
...
def sendmessage(url):
    msg = input(f"{yellow}[? XYLOHOOK ?]{white} Message: ")
    try:
        response = requests.post(url, json={"content": msg})
        response.raise_for_status()
        print(f"{green}[+ XYLOHOOK +]{white} Message sent successfully.")
...
def renamehook(url):
    name = input(f"{yellow}[? XYLOHOOK ?]{white} Webhook Name: ")
    print(f"{cyan}[+ XYLOHOOK +]{white} Trying to change username...")
    try:
        response = requests.patch(url, json={"name": name})
        response.raise_for_status()
        print(f"{green}[+ XYLOHOOK +]{white} Webhook name changed successfully.")
...
def spamhook(url):
    print(f"{cyan}[+ XYLOHOOK +]{white} Trying to spam webhook...")
    msg = input(f"{yellow}[? XYLOHOOK ?]{white} Spam Text: ")
    timeout = float(input(f"{yellow}[? XYLOHOOK ?]{white} Timeout (to avoid api-ratelimit): "))
    try:
        print(f"{red}[! XYLOHOOK !] Spam has started, Relaunch the tool to stop spam and use it again.")
        while True:
            response = requests.post(url, json={"content": msg})
            response.raise_for_status()
            print(f"{green}[+ XYLOHOOK +]{white} Sent message")
            time.sleep(timeout)
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! XYLOHOOK !] HTTP Error: {errh}")
...
webhook = {}
os.system("title github.com/Solvixs")
while True:
    clear()
    printascii()
    while True:
    ...
    while True:
        ...
        print(f"\n\n\n{green}[+ XYLOHOOK +]{white} Logged into webhook: {webhook_name}")
        ...
        if ch == 1:
            ...
            print(f"{cyan}[+ XYLOHOOK +]{white} Source code can be found here:")
            for platform, info in socials.items():
                link = info["link"].replace("https://", "")
